...
async function main() {
  ...
  const USDTToken = await ethers.getContractFactory("USDTTokenUpgradeable");
  const usdt = await upgrades.deployProxy(USDTToken, [deployer.address], {
  ...
  const nft = await upgrades.deployProxy(DatasetNFT, [deployer.address], {
    initializer: 'initialize',
    kind: 'uups'
  });
  ...
  // 4. Deploy Factory contract with token addresses
  ...
  // 5. Set factory address in NFT contract
  ...
  console.log("\nUSDT Token:");
  console.log("Proxy:", usdtAddress);
  console.log("Implementation:", await upgrades.erc1967.getImplementationAddress(usdtAddress));
  console.log("Admin:", await upgrades.erc1967.getAdminAddress(usdtAddress));
  console.log("Initial Supply:", await usdt.totalSupply());
